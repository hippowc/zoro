...
func main() {
	...
	err := wails.Run(&options.App{
		Title: "zoro",
		Width: 780,
		// 初值取「只有搜索框」时的高度：JS 万一没跑起来，窗口是个小搜索框而不是一整块
		// 透明空框；正常情况下首帧就被前端 ResizeObserver 纠正（app.js fitWindowToContent）。
		Height:   76,
		MinWidth: 560,
		// NSWindow 的 userMinSize 由 MinHeight 初始化，会静默夹住 JS 下发的 WindowSetSize，
		// 因此必须 <= app.js 的 MIN_WINDOW_HEIGHT，且只能在这里改：从 JS 调 WindowSetMinSize
		// 走的是 darwin adjustWindowSize()，它不重新锚定顶边，窗口会明显跳一下。
		MinHeight:         60,
		Frameless:         true,
		AlwaysOnTop:       true,
